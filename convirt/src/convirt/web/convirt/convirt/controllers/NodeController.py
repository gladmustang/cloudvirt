...
#    @expose()
    ...
    #            doms=self.node_service.import_vm_config(session['auth'],node_id, directory, filenames)
                ...
                return "{success:false,msg:'VM licenses are not enough'}"
        except Exception, ex:
            print_traceback()
...
#    @expose()
    ...
    def qcow2_snapshot_delete(self,dom_id, node_id,snapshot_tag):
        self.authenticate()
        try:
            self.node_service.qcow2_snapshot_delete(session['auth'],dom_id,node_id,snapshot_tag)
        except Exception, ex:
            print_traceback()
            return dict(success=False,msg= to_str(ex))
    ...
    def vm_config_settings(self,image_id,config,mode,node_id=None,group_id=None,dom_id=None,vm_name=None,date=None,time=None,_dc=None):
        ...
        try:
            ...
            if mode=='PROVISION_VM':
                ...
                if get_remain_vm_licenses(session['auth'].user.user_name) < len(vm_list):
